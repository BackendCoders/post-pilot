...
import type { ScrapedPageData, ISeoReport } from '../../../types';
...
const buildPdfContent = (doc: jsPDF, results: ScrapedPageData[], reports: Record<string, ISeoReport> = {}): void => {
  ...
  const allIssues = results.flatMap(p => {
    const report = p.analysisReport || (reports[p.url] as any);
    if (!report) return [];
    return Object.values(report.sections).flatMap((s: any) => s.issues);
  ...
  results.forEach((page, index) => {
    const report = page.analysisReport || reports[page.url];
    ...
    if (report) {
      ...
      sectionKeys.forEach((key, i) => {
        ...
        const section = (report.sections as any)[key];
...
export const generateSeoReportBuffer = async (results: ScrapedPageData[], reports: Record<string, ISeoReport> = {}): Promise<Buffer> => {
  const doc = new jsPDF();
...
export const generateSeoReportBase64 = async (results: ScrapedPageData[], reports: Record<string, ISeoReport> = {}): Promise<string> => {
  const doc = new jsPDF();
  buildPdfContent(doc, results, reports);
